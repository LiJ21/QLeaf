...
bool cuda_policy_supported(const BenchmarkInput &input) {
  ...
  if (smem > 48u * 1024u) {
    cudaError_t attr_error;
    if constexpr (TPolicy::Persistent) {
      attr_error = cudaFuncSetAttribute(
          tree_kernels::traverse_persistent<
              TPolicy::BT, TPolicy::Transposed, TPolicy::Bitmask,
              TPolicy::Cached, TPolicy::CacheAllWaves, TPolicy::CrossReduce,
              TPolicy::SinglePoller, TPolicy::StageFeatures,
              TPolicy::SeqFlag, TPolicy::TPT, TPolicy::CacheNodeMajor,
              TPolicy::NodeStride>,
  ...
  if constexpr (TPolicy::Persistent) {
    ...
    if (cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            ...
            tree_kernels::traverse_persistent<
                TPolicy::BT, TPolicy::Transposed, TPolicy::Bitmask,
                TPolicy::Cached, TPolicy::CacheAllWaves,
                TPolicy::CrossReduce, TPolicy::SinglePoller,
                TPolicy::StageFeatures, TPolicy::SeqFlag, TPolicy::TPT,
                TPolicy::CacheNodeMajor, TPolicy::NodeStride>,
            TPolicy::BT, smem) != cudaSuccess) {
      cudaGetLastError();
      return false;
    }
    int nsm = 0;
    ...
    if (per_sm <= 0 || grid > per_sm * nsm) return false;
  }
