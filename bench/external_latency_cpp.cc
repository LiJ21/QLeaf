...
struct Args {
  ...
  std::vector<std::string> nvforest_layouts{"depth_first", "layered",
                                            "breadth_first"};
  std::vector<std::string> nvforest_devices{"gpu", "cpu"};
  std::vector<size_t> nvforest_rows_per_block_iter{1, 2, 4, 8, 16, 32};
  std::vector<size_t> nvforest_cpu_chunk_sizes{64};
...
[[noreturn]] void usage(std::string_view message = {}) {
  ...
  std::cerr
      << "usage: external_latency_cpp --features N --features-csv file "
         "[options]\n\n"
      << "  --engine xgboost|treelite|nvforest|all\n"
      ...
      << "  --threads N                 TL2cgen predictor / nvForest CPU threads (default 1)\n"
      ...
      << "  --nvforest-devices a,b      gpu,cpu\n"
      << "  --nvforest-rows-per-block-iter 1,2,4,8,16,32\n"
      << "  --nvforest-cpu-chunk-sizes 64,128,...\n"
...
Args parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") usage();
      ...
      if (value == "all") {
        args.engines = {"xgboost", "treelite", "nvforest"};
      } else {
        args.engines = {value};
      }
    }
    ...
    else if (arg == "--nvforest-devices") args.nvforest_devices = parse_string_list(need_value(arg), arg);
    ...
    else if (arg == "--nvforest-cpu-chunk-sizes") {
      args.nvforest_cpu_chunk_sizes =
          parse_list<size_t>(need_value(arg), arg, [&](std::string_view v) {
            return parse_size(v, arg);
          });
    }
  ...
  for (const auto& engine : args.engines) {
    if (engine != "xgboost" && engine != "treelite" && engine != "nvforest") {
      usage("--engine/--engines supports xgboost, treelite, and nvforest");
    }
  }
  ...
  for (const auto& device : args.nvforest_devices) {
    if (device != "gpu" && device != "cpu") {
      usage("--nvforest-devices values must be gpu or cpu");
    }
  }
  ...
  for (size_t chunk_size : args.nvforest_cpu_chunk_sizes) {
    if (chunk_size == 0) {
      usage("--nvforest-cpu-chunk-sizes values must be positive");
    }
  }
  return args;
}
...
std::string nvforest_cpu_name(const std::string& layout, size_t chunk_size,
                              size_t align_bytes, int threads) {
  return "nvforest/cpu/threads:" + std::to_string(threads) +
         "/layout:" + layout +
         "/chunk_size:" + std::to_string(chunk_size) +
         "/align_bytes:" + std::to_string(align_bytes);
}

bool want_nvforest_device(const Args& args, std::string_view device) {
  return std::find(args.nvforest_devices.begin(), args.nvforest_devices.end(),
                   device) != args.nvforest_devices.end();
}

int nvforest_cpu_thread_count() {
#ifdef _OPENMP
...
void configure_nvforest_cpu_threads(int threads) {
#ifdef _OPENMP
  omp_set_dynamic(0);
...
std::unique_ptr<treelite::Model> build_treelite_xgb_json_model(
    const Args& args, const std::string& path) {
  std::ifstream in{path};
  if (!in) throw std::runtime_error("failed to open XGBoost JSON: " + path);
  nlohmann::json root;
  in >> root;
  ...
  model->postprocessor =
      args.output == "margin"
          ? "identity"
          : (objective == "binary:logistic" ? "sigmoid" : "identity");
...
nlohmann::json measure_nvforest_cpu_one(
    const Args& args, const std::vector<float>& features, size_t samples,
    const std::string& model_path, nvforest::forest_model& model,
    const std::string& layout_name, size_t chunk_size, size_t align_bytes) {
  const auto output_size = static_cast<size_t>(model.num_outputs());
  std::vector<float> host_output(output_size, 0.0f);
  raft_proto::buffer<float> output_buffer(
      host_output.data(), output_size, raft_proto::device_type::cpu, -1);

  std::vector<raft_proto::buffer<float>> input_buffers;
  input_buffers.reserve(samples);
  for (size_t row = 0; row < samples; ++row) {
    input_buffers.emplace_back(
        const_cast<float*>(features.data() + row * args.features),
        args.features, raft_proto::device_type::cpu, -1);
  }

  const auto chunk = std::optional<nvforest::index_type>{
      static_cast<nvforest::index_type>(chunk_size)};

  auto predict = [&](size_t row) -> double {
    model.predict<float>(output_buffer, input_buffers[row],
                         raft_proto::cuda_stream{},
                         nvforest::infer_kind::default_kind, chunk);
    return host_output.back();
  };

  const std::string name =
      nvforest_cpu_name(layout_name, chunk_size, align_bytes, args.threads);
  size_t row = 0;
  double last = 0.0;
  for (size_t i = 0; i < args.warmup; ++i) {
    last = predict(row % samples);
  ...
  run.update({
      ...
      {"engine", "nvforest"},
      {"device", "cpu"},
      {"threads", nvforest_cpu_thread_count()},
...
std::vector<nlohmann::json> run_nvforest(const Args& args,
  ...
  auto skip_gpu_all = [&](const std::string& reason) {
    if (!want_nvforest_device(args, "gpu")) return;
    for (const auto& layout : args.nvforest_layouts) {
  ...
  auto skip_cpu_all = [&](const std::string& reason) {
    if (!want_nvforest_device(args, "cpu")) return;
    for (const auto& layout : args.nvforest_layouts) {
      for (size_t align_bytes : args.nvforest_align_bytes) {
        for (size_t chunk_size : args.nvforest_cpu_chunk_sizes) {
          runs.push_back(skip_run(
              args, samples,
              nvforest_cpu_name(layout, chunk_size, align_bytes, args.threads),
              reason,
              {{"engine", "nvforest"},
               {"device", "cpu"},
               {"layout", layout},
  ...
  try {
    tl_model = build_treelite_xgb_json_model(args, model_path);
  } catch (const std::exception& e) {
    const std::string reason =
        std::string{"failed to prepare nvForest Treelite model: "} + e.what();
    skip_cpu_all(reason);
    skip_gpu_all(reason);
    return runs;
  }

  if (want_nvforest_device(args, "cpu")) {
    configure_nvforest_cpu_threads(args.threads);
#ifndef _OPENMP
    if (args.threads != 1) {
      skip_cpu_all(
          "external_latency_cpp was built without OpenMP; nvForest CPU cannot "
          "honor --threads > 1");
    } else
#endif
    {
          ...
          try {
            auto model = nvforest::import_from_treelite_model(
                *tl_model, parse_nvforest_layout(layout),
                ...
                std::optional<bool>{false}, raft_proto::device_type::cpu, -1);
            const auto num_features = static_cast<size_t>(model.num_features());
            if (num_features != args.features) {
              throw std::runtime_error(
                  "nvForest CPU model num_feature mismatch: model=" +
                  std::to_string(num_features) + " args=" +
                  std::to_string(args.features));
            }
            for (size_t chunk_size : args.nvforest_cpu_chunk_sizes) {
              try {
                runs.push_back(measure_nvforest_cpu_one(
                    args, features, samples, model_path, model, layout,
                    chunk_size, align_bytes));
              } catch (const std::exception& e) {
                runs.push_back(skip_run(
                    args, samples,
                    nvforest_cpu_name(layout, chunk_size, align_bytes,
                                      args.threads),
                    std::string{"failed to run nvForest CPU: "} + e.what(),
                    {{"engine", "nvforest"},
                     {"device", "cpu"},
                     {"layout", layout},
                     {"chunk_size", chunk_size},
            ...
            for (size_t chunk_size : args.nvforest_cpu_chunk_sizes) {
              runs.push_back(skip_run(
                  ...
                  nvforest_cpu_name(layout, chunk_size, align_bytes,
                                    args.threads),
                  std::string{"failed to prepare nvForest CPU model: "} +
                      e.what(),
                  {{"engine", "nvforest"},
                   {"device", "cpu"},
                   {"layout", layout},
  ...
  if (!want_nvforest_device(args, "gpu")) return runs;
  ...
  try {
    ...
    skip_gpu_all(std::string{"failed to initialize nvForest/CUDA: "} + e.what());
    return runs;
  }
  for (const auto& layout : args.nvforest_layouts) {
    for (size_t align_bytes : args.nvforest_align_bytes) {
...
std::vector<nlohmann::json> run_nvforest(const Args& args,
  ...
  if (want_nvforest_device(args, "gpu")) {
    for (const auto& layout : args.nvforest_layouts) {
      for (size_t rows_per_block_iter : args.nvforest_rows_per_block_iter) {
        for (size_t align_bytes : args.nvforest_align_bytes) {
          for (const auto& mode : args.nvforest_modes) {
    ...
    nlohmann::json root = {
        {"meta",
         {{"clock", "CLOCK_MONOTONIC_RAW"},
          ...
          {"threads", args.threads},
          {"ntrees", args.ntrees == 0 ? nullptr : nlohmann::json(args.ntrees)},
          {"nvforest_devices", args.nvforest_devices},
          {"nvforest_cpu_chunk_sizes", args.nvforest_cpu_chunk_sizes}}},
        {"runs", runs},
    };
