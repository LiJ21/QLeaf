...
[[noreturn]] void usage(std::string_view message = {}) {
  ...
  std::cerr
      ...
      << "  --gpu-suite best|sp|sp-map|mp-sentinel|bitmask-tpt|bitmask-cache|all|oneshot|naive|ablation\n"
...
Args parse_args(int argc, char **argv) {
  ...
  if (args.gpu_suite != "best" && args.gpu_suite != "sp" &&
      args.gpu_suite != "sp-map" && args.gpu_suite != "mp-sentinel" &&
      args.gpu_suite != "all" && args.gpu_suite != "bitmask-tpt" &&
      args.gpu_suite != "bitmask-cache" && args.gpu_suite != "oneshot" &&
      args.gpu_suite != "naive" && args.gpu_suite != "ablation") {
    usage("--gpu-suite must be best, sp, sp-map, mp-sentinel, all, "
          "bitmask-tpt, bitmask-cache, oneshot, naive, or ablation");
  }
