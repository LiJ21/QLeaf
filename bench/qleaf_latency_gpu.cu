...
bool prepare_cuda_features(GpuInput &input) {
  if (!cuda_runtime_available()) return false;
  input.mapped_features.reserve(input.features.size());
  for (const auto &row : input.features) input.mapped_features.emplace_back(row);
  return true;
}

nlohmann::json config_for_gpu(const nlohmann::json &forest, size_t features,
                              int persistent_grid) {
...
bool want(const GpuLatencyRequest &req, std::string_view name) {
  return req.filter.empty() ||
         std::string{name}.find(req.filter) != std::string::npos;
}

GpuLatencyRun skip_run(std::string name, std::string reason) {
  GpuLatencyRun run;
...
bool cuda_policy_supported(const GpuInput &input, int persistent_grid,
  ...
  if constexpr (TPolicy::Persistent) {
    auto *kernel = tree_kernels::traverse_persistent<
        TPolicy::BT, TPolicy::Transposed, TPolicy::Bitmask, TPolicy::Cached,
        TPolicy::CacheAllWaves, TPolicy::CrossReduce,
...
void run_gpu_policy(const GpuLatencyRequest &req, GpuInput &input,
  ...
  auto config_json = config_for_gpu(input.forest, req.n_features,
                                    req.persistent_grid);
...
template <int BT, bool Stage, bool Seq, template <typename> typename TNodeBuffer>
void run_sp_path(const GpuLatencyRequest &req, GpuInput &input,
                 std::vector<GpuLatencyRun> &out) {
  using P = qleaf::CudaTraversePersistentSP<
      BT, true, false, false, true, false, true, Stage, Seq>;
  run_gpu_policy<P, TNodeBuffer>(req, input, out);
}

template <int BT, bool CacheAllWaves,
          template <typename> typename TNodeBuffer>
void run_sp_cached_path(const GpuLatencyRequest &req, GpuInput &input,
                        std::vector<GpuLatencyRun> &out) {
  using P = qleaf::CudaTraversePersistentSP<
      BT, !CacheAllWaves, false, true, true, false, true, true, false, 1,
      CacheAllWaves>;
  run_gpu_policy<P, TNodeBuffer>(req, input, out);
}
...
void run_bitmask_tpt(const GpuLatencyRequest &req, GpuInput &input,
                     std::vector<GpuLatencyRun> &out) {
  using P = qleaf::CudaTraversePersistentSP<
      BT, Transposed, true, false, true, false, true, true, Seq, TPT, false,
      false, NodeStride>;
  run_gpu_policy<P, TNodeBuffer>(req, input, out);
}
...
void run_bitmask_cached_policy(const GpuLatencyRequest &req, GpuInput &input,
                               std::vector<GpuLatencyRun> &out) {
  using P = qleaf::CudaTraversePersistentSP<
      BT, Transposed, true, true, true, false, true, true, false, TPT,
      CacheAllWaves, CacheNodeMajor, NodeStride>;
  run_gpu_policy<P, TNodeBuffer>(req, input, out);
}
...
void run_tpt_value(const GpuLatencyRequest &req, GpuInput &input,
  ...
  switch (tpt) {
    ...
    case 16:
      ...
      run_bitmask_tpt_layout<BT, 16, false, false, TNodeBuffer>(req, input, out);
      break;
    case 32:
      ...
      run_bitmask_tpt_layout<BT, 32, false, false, TNodeBuffer>(req, input, out);
      break;
  }
}
...
template <int BT, int TPT, bool Transposed, int NodeStride,
          template <typename> typename TNodeBuffer>
void run_oneshot_cached_bitmask(const GpuLatencyRequest &req, GpuInput &input,
                                std::vector<GpuLatencyRun> &out) {
  using P = qleaf::CudaTraverseOneShot<
      BT, Transposed, true, true, false, TPT, false, NodeStride>;
  run_gpu_policy<P, TNodeBuffer>(req, input, out);
}

template <template <typename> typename TNodeBuffer>
void run_gpu_buffer(const GpuLatencyRequest &req, GpuInput &input,
                    std::vector<GpuLatencyRun> &out) {
  if (req.suite == "best" || req.suite == "sp") {
    run_sp_path<kCompiledBT, false, true, TNodeBuffer>(req, input, out);
    run_sp_path<kCompiledBT, true, true, TNodeBuffer>(req, input, out);
    run_sp_path<kCompiledBT, true, false, TNodeBuffer>(req, input, out);
  ...
  if (req.suite == "best" || req.suite == "bitmask-tpt") {
  ...
  if (req.suite == "oneshot") {
    run_oneshot_cached_bitmask<kCompiledBT, 1, true, 1, TNodeBuffer>(
        req, input, out);
    run_oneshot_cached_bitmask<kCompiledBT, 4, true, 4, TNodeBuffer>(
        req, input, out);
  }
  if (req.suite == "all") {
    out.push_back(skip_run("qleaf/gpu/exhaustive/bt:" +
                               std::to_string(kCompiledBT),
                           "qleaf_latency instantiates best/sp/bitmask-tpt; "
                           "use worker_bench for exhaustive policy sweeps"));
  }
}
...
std::vector<GpuLatencyRun> run_qleaf_gpu_latency(const GpuLatencyRequest &req) {
  std::vector<GpuLatencyRun> out;
  if (req.forest == nullptr || req.features == nullptr || req.features->empty()) {
  ...
  if (req.buffer == "compact") {
    run_gpu_buffer<qleaf::CompactNodeBuffer>(req, input, out);
  } else if (req.buffer == "tree") {
    run_gpu_buffer<qleaf::TreeNodeBuffer>(req, input, out);
  } else {
