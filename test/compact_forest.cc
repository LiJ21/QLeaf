...
TEST_F(CompactForestTest3, BothLeft)      { EXPECT_FLOAT_EQ(inf_->predict(std::vector<float>{0.3f, 0.3f}), 111.f); }
TEST_F(CompactForestTest3, LeftThenRight) { EXPECT_FLOAT_EQ(inf_->predict(std::vector<float>{0.3f, 0.7f}), 222.f); }
TEST_F(CompactForestTest3, RightThenLeft) { EXPECT_FLOAT_EQ(inf_->predict(std::vector<float>{0.7f, 0.3f}), 333.f); }
TEST_F(CompactForestTest3, BothRight)     { EXPECT_FLOAT_EQ(inf_->predict(std::vector<float>{0.7f, 0.7f}), 444.f); }

class CompactThreadedForestTest3 : public ::testing::Test {
protected:
  using Worker = qleaf::BranchRegressionWorker<float, qleaf::CompactNodeBuffer<float>::Span>;
  using Threaded = qleaf::ThreadedWorker<Worker>;
...
TEST_F(CompactThreadedForestTest3, BothLeft)      { EXPECT_FLOAT_EQ(predict({0.3f, 0.3f}), 111.f); }
TEST_F(CompactThreadedForestTest3, LeftThenRight) { EXPECT_FLOAT_EQ(predict({0.3f, 0.7f}), 222.f); }
TEST_F(CompactThreadedForestTest3, RightThenLeft) { EXPECT_FLOAT_EQ(predict({0.7f, 0.3f}), 333.f); }
TEST_F(CompactThreadedForestTest3, BothRight)     { EXPECT_FLOAT_EQ(predict({0.7f, 0.7f}), 444.f); }
...
TEST_F(CompactQuickScorerTest3, BothLeft)      { EXPECT_FLOAT_EQ(qs_->predict(std::vector<float>{0.3f, 0.3f}), 111.f); }
TEST_F(CompactQuickScorerTest3, LeftThenRight) { EXPECT_FLOAT_EQ(qs_->predict(std::vector<float>{0.3f, 0.7f}), 222.f); }
TEST_F(CompactQuickScorerTest3, RightThenLeft) { EXPECT_FLOAT_EQ(qs_->predict(std::vector<float>{0.7f, 0.3f}), 333.f); }
TEST_F(CompactQuickScorerTest3, BothRight)     { EXPECT_FLOAT_EQ(qs_->predict(std::vector<float>{0.7f, 0.7f}), 444.f); }
